...
class PokeListWidget extends StatefulWidget {
  const PokeListWidget({
    ...
    int? pokemonsLenght,
    int? pokemonIndex,
  })  : pokemonsLenght = pokemonsLenght ?? 250,
        pokemonIndex = pokemonIndex ?? 0;

  final int pokemonsLenght;
...
class _PokeListWidgetState extends State<PokeListWidget>
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      while (widget.pokemonsLenght == widget.pokemonIndex) {
        _model.apiResultk2r = await PokeApiGroup.pokemonCall.call(
          pokemonId: widget.pokemonIndex,
        );
        if ((_model.apiResultk2r?.succeeded ?? true)) {
          await actions.addPokemons(
            context,
            PokemonStruct(),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                (_model.apiResultk2r?.exceptionMessage ?? ''),
                style: TextStyle(
                    ...
                    Padding(
                      ...
                      child: Text(
                        valueOrDefault<String>(
                          FFAppState().pokeList.length.toString(),
                          '0',
                        ),
                        style: FlutterFlowTheme.of(context).bodyMedium.override(
                              fontFamily: 'Readex Pro',
